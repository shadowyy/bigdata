    ...
    protected static class FlushThread extends Thread {
        ...
        private void flushToHBase() throws IOException {
            synchronized (hTable) {
                ...
                for (Map.Entry<String, CounterMap> entry : rowKeyCounterMap.entrySet()) {
                    ...
                    for (Map.Entry<String, CounterMap.Counter> entry2 : pastCounterMap.entrySet()) {
                        increment.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(entry2.getKey()), entry2.getValue().value);
                        hasColumns = true;
                    }
                    if (hasColumns) {
                        updateLastUsed();
